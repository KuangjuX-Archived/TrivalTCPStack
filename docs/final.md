...
为了测试程序的极限性能，我们没有设置丢包率，我们发送大量的数据（小说《天龙八部》第一章），测试结果如上图所示：

我们在20s的时间内发送了大约 52000 bytes 的数据并成功反馈了 ACK，吞吐量为：

52000bytes / 20s = 2600 bytes/s

在我们的设计中，我们的用户线程负责调用用户 API，而在内核中我们新开了两个线程，分别为发送线程（不断轮询发送队列的数据进行发送）以及计时器（挂载回调函数并计时），因此我们在传输过程中不会造成程序的阻塞，因此拥有较好的性能。

存在的问题：

- 随着工程量的增大，代码结构较为混乱，调试起来较为困难，如果有时间给将会重构工程
- 接收方服务器回复 ACK 的速度较慢，成为制约程序性能的一大原因
- 部分协议设计不够简单，处理起来较为复杂，在分布式情况下调试起来也较为困难
